	...
	<visual>
		<div class="block">
			<?php
   			...
   			$date = date($format='Y-m-d H:i:s');
   			$query = $connect->query($statement= "INSERT INTO nerodro.verstka (username, comment, date) VALUES('$username','$comment', '$date')");
			...
			<div class="content">
				<?php
				$post = $connect->query($statement="SELECT * FROM nerodro.posts WHERE `id` = 28");
          ...
          <hr>
			...
			<div class="hoverr">
			  <p>Ваше имя:<br></p>
   			...
   			<p>Сообщение</br>
   			<textarea name="comment" cols="40" rows="3" minlength="5" maxlength="100" required></textarea></p>
   			  <input class="btn" type="submit" value="Отправить"/>
   			</form>
   			<?php
   			$verstka = $connect->query($statement="SELECT * FROM nerodro.verstka ORDER BY date DESC");
   			$verstka = $verstka->fetchALL($fetch_style = PDO ==FETCH_ASSOC);
   			
   			if ($verstka ){
   			foreach ($verstka as $comment ) {
   			?>
