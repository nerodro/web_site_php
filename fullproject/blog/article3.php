	...
	<visual>
		<div class="block">
			<?php
   			...
   			$date = date($format='Y-m-d H:i:s');
   			$query = $connect->query($statement= "INSERT INTO nerodro.comments (username, comment, date) VALUES('$username','$comment', '$date')");
			...
			<div class="content">
				<?php
				$post = $connect->query($statement="SELECT * FROM nerodro.posts WHERE `id` = 30");
          ...
          <hr>
			...
			<div class="hoverr">
			  <p>Ваше имя:<br></p>
   			...
   			<?php
   			$comments = $connect->query($statement="SELECT * FROM nerodro.comments ORDER BY date DESC");
   			$comments = $comments->fetchALL($fetch_style = PDO ==FETCH_ASSOC);
